...
const Register = () => {
  ...
  const onSubmitPhone = async (data) => {
    ...
    try {
      ...
      // Add the + prefix if not present (with India country code)
      if (!formattedPhone.startsWith("+")) {
        // If it already starts with 91, add the + prefix
        if (formattedPhone.startsWith("91")) {
          formattedPhone = "+" + formattedPhone;
        } else {
          // Otherwise, add the +91 prefix (assuming Indian numbers)
          formattedPhone = "+91" + formattedPhone;
        }
      }

      console.log("Sending OTP request with formatted phone:", formattedPhone);
      ...
      if (response && response.status === 200) {
        ...
        // If we're in dev mode, show an extra toast about checking server logs
        if (response.data && response.data.devMode) {
          toast.info("Running in dev mode - check server logs for the OTP");
        }
      } else {
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to send OTP. Please check your phone number format and try again."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const onSubmitOtp = async (data) => {
    ...
    try {
      ...
      if (response && response.status === 200) {
        // Store the token from cookies (handled by the browser)
        // If token is in the response data, save it as well
        ...
        setStep(3);
        toast.success(
          "OTP verified successfully. Complete your profile to continue."
        );
      } else {
  ...
  const resendOtp = async () => {
    try {
      ...
      if (!phoneNumber || !phoneNumber.startsWith("+")) {
        toast.error(
          "Invalid phone number format. Please go back and enter a valid number."
        );
        return;
      }
      ...
      toast.error(
        error.response?.data?.message ||
          "Server error while resending OTP. Please try again later."
      );
    } finally {
      setIsLoading(false);
    }
            ...
            >
              <div>
                ...
                <div className="mt-1">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Enter only digits - country code (+91) will be added
                    automatically
                  </p>
                </div>
                {phoneErrors.phoneNumber && (
              ...
              <div>
                ...
                <p className="mt-2 text-xs text-center text-gray-500">
                  In development mode, the OTP will appear in the server logs
                </p>
              </div>
            </form>
          )}
