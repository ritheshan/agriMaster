...
const Login = () => {
  ...
  const onSubmitPhone = async (data) => {
    ...
    try {
      ...
      console.log("This is formatted: ", formattedPhone);
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (response.data.otp) {
          ...
          console.log(
            "%c 🔑 DEVELOPMENT OTP: " + response.data.otp,
            "background: #222; color: #bada55; font-size: 16px; padding: 10px;"
          );

          toast.info(
            <div className="text-center">
              <div className="text-xl font-bold">DEV MODE: New OTP</div>
              <div className="text-3xl mt-2 bg-gray-100 p-2 rounded">
                {response.data.otp}
              </div>
            </div>,
            {
          ...
          if (response.data.twilioError) {
            toast.warning(
              `Note: SMS not sent (${response.data.twilioError}), but you can use the displayed OTP for testing`
            );
          }
        } else {
